...
- **GrainType Validation**: 
  ...
  - Силосы с индексами 1, 2, 19, 25, 26, 27 исключены из проверки.
  - `Force_Code` (BIT10) позволяет игнорировать несовпадение типов зерна.
- **Reject vs Abort**:
...
- **Safety**: `Emergency_Stop` взводит латч `GlobalSafetyStop`. Пока он активен, `Enable_OK` всех механизмов = FALSE.
- **Safety Reset**: Сброс `GlobalSafetyStop` возможен только через `GlobalReset` при условии, что `Emergency_Stop` отключен.
- **Global Reset**: Тэг `GlobalReset` (Bool) — рассылает `CMD_RESET` всем механизмам (слоты 1..319) и автоматически сбрасывается.
...
## 🌪 Зерносушилка (Dryer)
### Структура данных (`UDT_Dryer` v0.4)
...
### Логика управления (`FC_Dryer`)
- **Пуск (Starting)**: Последовательный запуск Fans(1-7) -> Burners(1-2) с задержкой 3с.
 **Варианты**: 
  - `FC_Dryer`: Стандартная (2 горелки).
  - `FC_Dryer_1B`: Облегченная (1 горелка, используется только `Burners[1]`).
 **Пуск (Starting)**: Последовательный запуск Fans(1-7) -> Burners (с задержкой 3с).
